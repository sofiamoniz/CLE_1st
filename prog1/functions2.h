...
char convert_multibyte_char(wchar_t c){
    switch (c) {
...
int is_space_separation_punctuation_char(unsigned char c){
    if(c==' ' || c =='  ' || c==0xa){ //space
        return 1;
    }
...
int is_apostrophe_char(unsigned char c){
    if(c==0x27){
        return 1;
    }
    else{
...
int size_of_array(char **words_array){
    int i = 0;
